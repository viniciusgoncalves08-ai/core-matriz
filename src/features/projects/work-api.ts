...
export async function workResponse(action: (userId: string) => Promise<unknown>, status = 200) {
  ...
  try { return NextResponse.json(await action(userId), { status }); }
    ...
    if (error instanceof WorkNotFoundError) return NextResponse.json({ error: "Registro não encontrado." }, { status: 404 });
    if (error instanceof z.ZodError || error instanceof SyntaxError) return NextResponse.json({ error: "Confira os campos: título, situação, progresso e prazo devem ser válidos." }, { status: 400 });
