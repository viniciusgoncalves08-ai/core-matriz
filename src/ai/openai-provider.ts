...
export function supportsTemperature(model: string) {
  // Reasoning models use their default reasoning settings; do not send sampling parameters.
  ...
  async generate(input: GenerateInput): Promise<GenerateResult> {
    ...
    try {
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({ model, messages: input.messages, ...(supportsTemperature(model) ? { temperature: input.temperature ?? 0.2 } : {}) }),
      });
