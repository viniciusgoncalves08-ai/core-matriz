...
const mock = vi.hoisted(() => ({ task: { count: vi.fn().mockResolvedValue(0), findMany: vi.fn().mockResolvedValue([]) }, project: { count: vi.fn().mockResolvedValue(0), findMany: vi.fn().mockResolvedValue([]) }, goal: { findMany: vi.fn().mockResolvedValue([]) }, memory: { count: vi.fn().mockResolvedValue(0) } }));
...
it("scopes every overview query to the user and excludes closed tasks", async () => {
  ...
  for (const fn of [mock.task.count, mock.task.findMany, mock.project.count, mock.project.findMany, mock.memory.count, mock.goal.findMany]) {
