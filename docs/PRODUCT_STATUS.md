...
| Permissões / ações / tools (24–26) | Propostas de tarefas/projetos por comando explícito, confirmação autenticada, cancelamento, expiração, idempotência e auditoria | Motor genérico de permissões e novas ferramentas. Não há ferramentas externas funcionando |
| Projetos / tarefas (27–30) | Telas, APIs, vínculo tarefa–projeto, prazos, prioridade, situações, arquivo/restauração de projeto, auditoria | Responsáveis, tags, dependências, relações, histórico visível, interpretação livre de intenção, edição pelo chat e políticas para tarefas implícitas |
